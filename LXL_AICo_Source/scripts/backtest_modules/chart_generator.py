...
class ChartGenerator:
    ...
    def _generate_performance_comparison(self, metrics=None):
        ...
        try:
            ...
            ax1.plot(cum_benchmark.index, cum_benchmark.values * 100,
            ...
            # 专门为cumulative returns图表优化横轴显示
            self._optimize_cumulative_returns_axis(ax1, cum_strategy.index)
    ...
    def _optimize_cumulative_returns_axis(self, ax, dates):
        """专门优化cumulative returns图表的横轴显示"""
        try:
            ...
            # 根据数据长度选择合适的日期格式和刻度间隔
            date_range = dates.max() - dates.min()
            
            # 根据数据点数量调整刻度密度
            n_points = len(dates)
            
            if n_points > 500:  # 数据点非常多
                # 使用季度或半年刻度
            ...
            if len(dates) > 0:
                ...
                # 在轴的两端添加日期范围标记
                ax.text(0.02, -0.15, f"Start: {start_date}", transform=ax.transAxes, 
                       fontsize=8, color='gray', alpha=0.7)
                ax.text(0.85, -0.15, f"End: {end_date}", transform=ax.transAxes, 
                       fontsize=8, color='gray', alpha=0.7)
            
        except Exception as e:
            print(f"⚠️ Cumulative returns axis optimization failed: {e}")
            # 备用方案：使用自动日期格式化，但强制限制刻度数量
